...
settings = {
    ...
    "man2":"ตั้งข้อความ",
    "man3":"ตั้งข้อความ",
    "message":"บัญชีนี้ได้การป้องกันโดย Ŧ€₳M↬฿❂Ŧ ─═हই♞ωபԾΩ ❼ஆ้ণ♞ইह═─🇹🇭  ระบบได้ทำการบล็อคคุณอัตโนมัติ\nติดต่อเชลบอทรึบอทวี10คอมเม้นในโพสเลยคับ",
...
#===============หาชื่อล่อนหน===============================================================#
                    ...
                    if targets == []:
                        line.sendMessage(to, "🤔ไม่เจอล่องหนครับเจ้านาย😂")
                    else:
                        mc = ""
                    ...
                    for g in gs.members:
                        if g.displayName in "":
                            lists.append(g.mid)
                    if lists == []:
                        line.sendMessage(to, "🤗ไม่มีmidคนใส่ร่องหนครับเจ้านาย🤗")
                    else:
                        mc = ""
                        for mi_d in lists:
                            mc += "->" + mi_d + "\n"
                    ...
                    for g in gs.members:
                        if g.displayName in "":
                            lists.append(g.mid)
                    if lists == []:
                        line.sendMessage(to, "🤔ไม่มีคนใส่ร่องหนในกลุ่มนี้ครับเจ้านาย😂")
                    else:
                        for ls in lists:
                    ...
                    if LINE != None:
                         ...
                         if spl[0] == "":
                            ...
                            if msg.toType == 2:
                                ...
                                num = int(strnum)
                                line.sendMessage(to, "มาเล่นกันเถอะๆ(｀・ω・´)")
